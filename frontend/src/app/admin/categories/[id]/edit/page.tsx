...
import { Tag, ArrowLeft, Save, Shield, Loader2 } from "lucide-react"
...
export default function EditCategoryPage() {
  ...
  const loadCategory = async () => {
    try {
      ...
      const foundCategory = response.category
  ...
  const onSubmit = async (data: CategoryForm) => {
    ...
    try {
      ...
      const message = error.response?.data?.error || "Có lỗi xảy ra khi cập nhật danh mục"
      toast.error(message)
    } finally {
  ...
  if (!category) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Danh mục không tồn tại</p>
          <Link
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Cập nhật thông tin danh mục "{category.name}"</p>
          </div>
        </div>
